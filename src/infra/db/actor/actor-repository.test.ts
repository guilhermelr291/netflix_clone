...
describe('Actor Repository', () => {
  ...
  describe('update()', () => {
    ...
    test('should return an actor on success', async () => {
      const { sut } = makeSut();
      const result = await sut.update('1', mockUpdateActorParams());
      expect(result).toEqual(mockActor());
    });
    test('should call ActorMapper with correct values', async () => {
      const { sut, actorMapperStub } = makeSut();
    ...
    test('should throw if prisma throws', async () => {
      const { sut } = makeSut();
      vi.mocked(prisma.actor.update).mockRejectedValueOnce(new Error());
      await expect(sut.update('1', mockUpdateActorParams())).rejects.toThrow();
    });
  });
});
